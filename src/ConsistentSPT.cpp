    ...
    : C(C_), P0(P0_)
{
    ...
    vector<vector<real> > F = RungeKutta4(0, s1, f0, bind(&ConsistentSPT::dfds, this, _1, _2), Nsteps);
    U    = CubicSpline(S, F[0]);
...
real ConsistentSPT::P_AA(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(f_AA, cref(P0), k, _1, _2), a, b, epsrel);
}
real ConsistentSPT::P_AB(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(f_AB, cref(P0), k, _1, _2), a, b, epsrel);
}
real ConsistentSPT::P_BB(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(f_BB, cref(P0), k, _1, _2), a, b, epsrel);
}
real ConsistentSPT::P_A0A(real k) const {
    return -A*k*k*P0(k);
}
real ConsistentSPT::P_AA0(real k) const {
    return k*k/(4*M_PI*M_PI) * P0(k) * Integrate<ExpSub>(bind(c_AA0, cref(P0), k, _1), QMIN, QMAX, epsrel);
}
...
real ConsistentSPT::P_B0A(real k) const {
    return k*k/(4*M_PI*M_PI) * P0(k) * Integrate<ExpSub>(bind(c_B0A, cref(P0), k, _1), QMIN, QMAX, epsrel);
}
