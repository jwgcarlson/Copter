...
real LargeN::Sigma_11(real k) const {
    return k*k/(4*M_PI*M_PI) * Integrate<ExpSub>(bind(f_11, cref(P_L0), k, _1), QMIN, QMAX, epsrel);
}

real LargeN::Sigma_12(real k) const {
    return k*k/(4*M_PI*M_PI) * Integrate<ExpSub>(bind(f_12, cref(P_L0), k, _1), QMIN, QMAX, epsrel);
}

real LargeN::Sigma_21(real k) const {
    return k*k/(4*M_PI*M_PI) * Integrate<ExpSub>(bind(f_21, cref(P_L0), k, _1), QMIN, QMAX, epsrel);
}

real LargeN::Sigma_22(real k) const {
    return k*k/(4*M_PI*M_PI) * Integrate<ExpSub>(bind(f_22, cref(P_L0), k, _1), QMIN, QMAX, epsrel);
}
...
real LargeN::Pi_11(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(g_11, cref(P_L0), k, _1, _2), a, b, epsrel);
}

real LargeN::Pi_12(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(g_12, cref(P_L0), k, _1, _2), a, b, epsrel);
}

real LargeN::Pi_22(real k) const {
    real a[2] = { QMIN, -1 };
    real b[2] = { QMAX, XMAX };
    return k*k/(8*M_PI*M_PI) * Integrate<2>(bind(g_22, cref(P_L0), k, _1, _2), a, b, epsrel);
}
...
void LargeN::ComputeR(real k, real eta1, int Neta, array& R_11, array& R_12, array& R_21, array& R_22) const {
    ...
    for(int n = 0; n < Neta; n++) {
        ...
        x0[5] = -0.75;
        X = RungeKutta4(eta2, eta1, x0, bind(dXdeta1, _1, _2, s11, s12, s21, s22), Neta);
        R_11[n] = X[0].back();
        R_21[n] = X[1].back();
