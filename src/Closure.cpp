...
real Closure::P2_11(real k) const {
    ...
    real a[2] = { QMIN, -1 };
    real b[2] = { kcut, +1 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::f2_11, cref(*this), k, _1, _2), a, b, 1e-3, 1e-3*P_L(k));
#endif
    real a[2] = { k/M_SQRT2, 0 };
    real b[2] = { kcut, k/M_SQRT2 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::F2_11, cref(*this), k, _1, _2), a, b, 1e-3, 1e-3*P_L(k)/2);
}
real Closure::P2_12(real k) const {
#if 0
    real a[2] = { QMIN, -1 };
    real b[2] = { kcut, +1 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::f2_12, cref(*this), k, _1, _2), a, b, 1e-4, 1e-4*P_L(k));
#endif
    real a[2] = { k/M_SQRT2, 0 };
    real b[2] = { kcut, k/M_SQRT2 };
    return (k == 0) ? 0 : 2*Integrate<2>(bind(&Closure::F2_12, cref(*this), k, _1, _2), a, b, 1e-3, 1e-3*P_L(k)/2);
}
